...
SELECT * FROM (
    ...
    UNION ALL SELECT UUID(), 1, 'Leo cầu thang', 'Stair Climbing', 'Leo cầu thang', 8.8, 'minutes', NULL, UTC_TIMESTAMP(), UTC_TIMESTAMP()
    UNION ALL SELECT UUID(), 1, 'Aerobic', 'Aerobic', 'Aerobic cường độ trung bình', 7.0, 'minutes', NULL, UTC_TIMESTAMP(), UTC_TIMESTAMP()
    UNION ALL SELECT UUID(), 1, 'Chạy bộ nhanh', 'Fast Running', 'Chạy bộ tốc độ cao', 12.0, 'minutes', NULL, UTC_TIMESTAMP(), UTC_TIMESTAMP()
) AS tmp
WHERE NOT EXISTS (
    SELECT 1 FROM exercises WHERE NameVi = tmp.NameVi AND CategoryId = tmp.CategoryId
);

-- Seed Exercises - Sức mạnh (7 bài)
INSERT INTO exercises (Id, CategoryId, NameVi, NameEn, Description, MetValue, Unit, IconUrl, CreatedAt, UpdatedAt)
SELECT * FROM (
    ...
    UNION ALL SELECT UUID(), 2, 'Burpees', 'Burpees', 'Burpees', 10.0, 'minutes', NULL, UTC_TIMESTAMP(), UTC_TIMESTAMP()
) AS tmp
WHERE NOT EXISTS (
    SELECT 1 FROM exercises WHERE NameVi = tmp.NameVi AND CategoryId = tmp.CategoryId
);
...
-- Seed Exercises - Khác (4 bài)
INSERT INTO exercises (Id, CategoryId, NameVi, NameEn, Description, MetValue, Unit, IconUrl, CreatedAt, UpdatedAt)
SELECT * FROM (
    SELECT UUID() AS Id, 5 AS CategoryId, 'Làm vườn' AS NameVi, 'Gardening' AS NameEn, 'Làm vườn' AS Description, 4.0 AS MetValue, 'minutes' AS Unit, NULL AS IconUrl, UTC_TIMESTAMP() AS CreatedAt, UTC_TIMESTAMP() AS UpdatedAt
    UNION ALL SELECT UUID(), 5, 'Dọn dẹp nhà', 'House Cleaning', 'Dọn dẹp nhà', 3.5, 'minutes', NULL, UTC_TIMESTAMP(), UTC_TIMESTAMP()
    UNION ALL SELECT UUID(), 5, 'Nấu ăn', 'Cooking', 'Nấu ăn', 2.5, 'minutes', NULL, UTC_TIMESTAMP(), UTC_TIMESTAMP()
